...
RouteModel::Node *RoutePlanner::NextNode() 
{
    ...
    RouteModel::Node* lastNode = this->open_list.back();
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    //Add end node to the vector
    path_found.push_back(*current_node);
    while(current_node != this->start_node)
    {
        //Extract parent and add to vector
        path_found.push_back(*(current_node->parent)); 
        //Calculate the distance of current node to parent node
        this->distance += current_node->distance(*(current_node->parent));
        //Replace current node with parent node to traverse back towards start node
        current_node = current_node->parent;
    }

    //Order the vector with start node as first element
